...
class Parser {
        ...
        sequenceNode.childNodes.forEachXsdElement { sequenceChildren ->
            when (sequenceChildren.nodeName) {
                XsdElementNames.ELEMENT.value -> {
                    ...
                    elementComplete.classSuffix = reference.firstToUpper()
                }

                XsdElementNames.GROUP.value -> {
                    elementComplete.addValue(sequenceChildren)
                }
            }
        }
    }
            ...
            val simpleType =
                SimpleType(
                    ...
                    restriction.attributes
                        .getNamedItem(AttributeKeyName.BASE.value)
                        .nodeValue
                        .substring(xsdStrLength),
                )
    ...
    private fun createComplexType(current: Node) {
        current.childNodes.getFirstNodeWithType(groupSet)?.let { group ->
            val choice =
                Choice(current.attributes.getNamedItem(AttributeKeyName.NAME.value).nodeValue, ClassNames.CHOICE.value)
